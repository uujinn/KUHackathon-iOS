...
class VoiceViewModel : NSObject, ObservableObject , AVAudioPlayerDelegate{
  ...
  func startRecording() {
    ...
    let recordingSession = AVAudioSession.sharedInstance() // 싱글톤 인스턴스 획득
    do {
      try recordingSession.setCategory(.playAndRecord, mode: .default) // 오디오 세션 카테고리, 모드, 옵선 설정
    ...
    let fileName = path.appendingPathComponent("Audio File : \(Date().toString(dateFormat: "dd-MM-YY 'at' HH:mm:ss")).m4a")
